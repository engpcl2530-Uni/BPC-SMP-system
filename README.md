...
*   **`applyFilters()`:** ระบบค้นหาและคัดกรองข้อมูล (ฝั่ง Client-side) ทำงานแบบ Multi-conditional (คัดกรองวันที่, ชื่อ, ไลน์, ประเภท พร้อมกัน)
...
*   **API Connection Failure (ขึ้นแจ้งเตือนเชื่อมต่อฐานข้อมูลไม่ได้)**
    *   *Solution:* หากมีการแก้ไขโค้ดในฝั่ง Google Apps Script (`Code.gs`) ต้องกด `Deploy -> New Deployment` ทุกครั้ง และนำ Web App URL ใหม่มาอัปเดตที่ตัวแปร `const API_URL = "..."` ในไฟล์ `script.js`

*   **Mobile Camera Override (มือถือบังคับเปิดกล้อง เลือกรูปจากอัลบั้มไม่ได้)**
    *   *Solution:* ตรวจสอบใน `index.html` หรือ `script.js` ตรงปุ่ม `<input type="file">` สำหรับเลือกอัลบั้ม ต้องไม่มีคำสั่ง `capture="environment"` ติดอยู่
